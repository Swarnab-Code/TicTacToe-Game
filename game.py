...
from draw import draw_lines, draw_figures, draw_vertical_winning_line, draw_horizontal_winning_line, draw_desc_diagonal, draw_asc_diagonal
from ai import AI
...
class Game:
	def __init__(self, screen):
		...
		self.board = Board()
		self.ai = AI()
		self.gamemode = 'ai' # set initial mode to 'ai'
		self.running = True

	def play(self, row, col):
		if self.board.available_square(row, col):
			# Mark the square for the current player
			self.board.mark_square(row, col, self.board.player)
			draw_figures(self.screen, self.board)
